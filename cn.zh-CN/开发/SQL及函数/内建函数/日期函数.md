...
-   参数说明
    ...
    -   `datepart`：STRING类型常量。此字段的取值遵循STRING与DATETIME类型转换的约定，即yyyy表示年，mm表示月。 关于类型转换的规则，请参见[STRING类型与DATETIME类型之间的转换](cn.zh-CN/开发/SQL及函数/SQL概述.md)。此外也支持扩展的日期格式：年`-year`、月`-month`或`-mon`、日`-day`、小时`-hour`。非常量、不支持的格式或其它类型会抛异常。
    **说明：** 
...
关于其他类型向STRING类型转换的详情请参见[字符串函数\>TO\_CHAR](cn.zh-CN/开发/SQL及函数/内建函数/字符串函数.md)。
