...
|----|--------------|-------------|
|操作描述| 准确的创建角色并配置角色权限，及时清理离职或转岗人员的账号，清理角色中不必要开放的资源和权限。
...
2.  将Package赋权给用户、角色，Pckage赋权给具体用户时，不能指定Label（Project Owner或Admin都可操作）：grant actions on package <pkgName\> to user <username\>;grant actions on package <pkgName\> to role <role\_name\>;。
 ...
 actions类型：请参见[授权](cn.zh-CN/管理/安全功能详解/用户及授权管理/授权.md#)，通常将Package的Read权限赋给对象，即可满足对象访问Package中资源的需求。
...
-   查看一个用户对一个数据表的所有列级别的Label权限：SHOW LABEL \[<level\>\] GRANTS ON TABLE <tablename\> FOR USER <username\>;。

--显示指定用户对指定Table上列级别的[Label 授权](cn.zh-CN/管理/安全功能详解/列级别访问控制.md#)。


 |
|赋权操作| -   给用户单个表或字段的安全许可标签：GRANT LABEL <number\> ON TABLE <tablename\>\[\(column\_list\)\] TO USER <username\> \[WITH EXP <days\>\];。
...
-   控制Package安装者对Package中敏感资源的许可访问级别：ALLOW PROJECT <prjName\> TO INSTALL PACKAGE <pkgName\> \[USING LABEL <number\>\];。
