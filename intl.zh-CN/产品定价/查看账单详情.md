...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/156894543152783_zh-CN.png)
...
点击**详情**查看每个项目的消费情况，您还可以通过下载[使用记录](#section_ug2_07v_iu0)查看消费是如何产生的。
...
-   如果您想知道**如何计算某个SQL任务的费用**，可以在下载使用记录后，打开CSV表格。查看**数据分类**中的**ComputationSql**SQL计费项。某项目SQL任务产生的费用计算公式为`计算输入数据量 * SQL复杂度 * SQL价格`。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15689454319592_zh-CN.png)

    如果您使用了[外部表功能](../../../../intl.zh-CN/开发/外部表/外部表概述.md#)，例如OTS、OSS外表，则上述表格中还会出现SQL读取量\_访问OTS（Byte）或SQL读取量\_访问OSS（Byte）信息。外表的计费公式如下。

    ``` {#codeblock_527_2p7_264}
...
-   如果您想知道**费用产生的原因和存在的问题**，可复制`Log view`的URL后在浏览器打开。通过Log view可以发现全表扫描、长尾计算等问题，您可以参考[计算长尾优化](../../../../intl.zh-CN/最佳实践/计算优化/计算长尾调优.md#)进行调优。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/11938/15689454318899_zh-CN.png)
...
-   **分析数据上传和下载是否产生了费用** 
    ...
    3.  公网下行流量产生了一条约0.036GB（38199736byte）的下行流量，根据[下载计费（按量计费）](intl.zh-CN/产品定价/下载计费（按量计费）.md#)标准，\(38,199,736 bytes/1024/1024/1024\) x 0.1166USD/GB = 0.004USD。
...
-   **分析“不足一天”的数据存储费用** 
    ...
    **说明：** 天计量范围以结束时间为准，所以最后一条数据不包括在4月4日账单中。
    ...
    按照[存储计费项（按量计费）](intl.zh-CN/产品定价/存储计费项（按量计费）.md#)先计算24小时存储价格：
